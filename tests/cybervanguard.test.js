// Node.js Unit Tests for Cyber Vanguard Engine logic
...
// Extract CyberVanguardEngine from cybervanguard.html
const htmlPath = path.join(__dirname, '..', 'cybervanguard.html');
const fileContent = fs.readFileSync(htmlPath, 'utf8');

const startIndex = fileContent.indexOf('class CyberVanguardEngine {');
const endIndex = fileContent.indexOf('if (typeof module !== \'undefined\' && typeof module.exports !== \'undefined\')');
if (startIndex === -1 || endIndex === -1) {
  throw new Error('Could not find CyberVanguardEngine boundaries in cybervanguard.html');
}

const engineCode = fileContent.substring(startIndex, endIndex) + '\nmodule.exports = { CyberVanguardEngine };';
...
const CyberVanguardEngine = mockModule.exports.CyberVanguardEngine;

test('CyberVanguardEngine - Initial state', () => {
  const engine = new CyberVanguardEngine(null);
...
test('CyberVanguardEngine - Lifecycle controls & state resets', () => {
  const engine = new CyberVanguardEngine(null);
  engine.start();
...
test('CyberVanguardEngine - Player movement & boundary clamping', () => {
  const engine = new CyberVanguardEngine(null);
  engine.start();
...
test('CyberVanguardEngine - Weapon firing & EMP Vanguard Shockwaves', () => {
  const engine = new CyberVanguardEngine(null);
  engine.start();
...
test('CyberVanguardEngine - Enemy spawning & collision resolution', () => {
  const engine = new CyberVanguardEngine(null);
  engine.started = true;
  engine.enemies = [];
...
test('CyberVanguardEngine - Boss Wave Spawning', () => {
  const engine = new CyberVanguardEngine(null);
  engine.startWave(5);
