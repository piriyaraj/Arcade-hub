// Node.js Unit Tests for Cyber Phoenix Engine logic
...
// Extract CyberPhoenixEngine from cyberphoenix.html
const htmlPath = path.join(__dirname, '..', 'cyberphoenix.html');
const fileContent = fs.readFileSync(htmlPath, 'utf8');

const startIndex = fileContent.indexOf('class CyberPhoenixEngine {');
const endIndex = fileContent.indexOf('if (typeof module !== \'undefined\' && typeof module.exports !== \'undefined\')');
if (startIndex === -1 || endIndex === -1) {
  throw new Error('Could not find CyberPhoenixEngine boundaries in cyberphoenix.html');
}

const engineCode = fileContent.substring(startIndex, endIndex) + '\nmodule.exports = { CyberPhoenixEngine };';
...
const CyberPhoenixEngine = mockModule.exports.CyberPhoenixEngine;
...
test('CyberPhoenixEngine - Initial state', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  ...
  assert.strictEqual(engine.player.rebirthGauge, 0);
  assert.strictEqual(engine.drones.length, 0);
});

test('CyberPhoenixEngine - Start game & wave initialization', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
...
test('CyberPhoenixEngine - Player movement & boundary clamping', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
...
test('CyberPhoenixEngine - Dual solar photon cannon firing', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  ...
  // Dual wing cannons fire 2 bullets
...
test('CyberPhoenixEngine - Inferno Dash', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
...
test('CyberPhoenixEngine - Deploy Ember Drone', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
...
test('CyberPhoenixEngine - Activate Solar Shield Barrier', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();

  const initialEnergy = engine.player.energy;
  const success = engine.activateShield();
  ...
  // Shield absorbs hits and recharges energy and rebirth
  const prevEnergy = engine.player.energy;
  const prevRebirth = engine.player.rebirthGauge;
  engine.takeDamage(20);
  assert.strictEqual(engine.player.hp, 100);
  assert.ok(engine.player.energy > prevEnergy);
  assert.ok(engine.player.rebirthGauge > prevRebirth);
});

test('CyberPhoenixEngine - Solar Flare EMP Nova', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  ...
  engine.enemies.push({ x: 300, y: 200, hp: 40, maxHp: 40, radius: 14, vx: 0, vy: 0, scoreValue: 80, color: '#f59e0b' });
...
test('CyberPhoenixEngine - Automatic Solar Rebirth Trigger on Fatal Damage', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();

  // Charge rebirth to max
  engine.player.rebirthGauge = 100;
  engine.player.hp = 20;

  // Deal lethal damage
  engine.takeDamage(50);

  // Should trigger rebirth, restore HP, and reset gauge
...
test('CyberPhoenixEngine - Powerup Collection & Tri-Beam Spread', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  ...
  engine.checkCollisions();
  assert.strictEqual(engine.player.rebirthGauge, 60);
...
test('CyberPhoenixEngine - Boss Wave Spawning & Defeat', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  ...
  assert.strictEqual(engine.boss.name, 'SOLAR ARCHON MK-1');
...
test('CyberPhoenixEngine - Game Pause & Resume', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
...
test('CyberPhoenixEngine - Game Over & High Score Persistence', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  ...
  assert.strictEqual(localStorage.getItem('cyberphoenix_best'), '9500');
});

test('CyberPhoenixEngine - Canvas drawing does not throw', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
  engine.spawnEnemy();
...
test('CyberPhoenixEngine - Resilience to NaN and edge cases', () => {
  const canvas = createMockCanvas();
  const engine = new CyberPhoenixEngine(canvas);
  engine.start();
