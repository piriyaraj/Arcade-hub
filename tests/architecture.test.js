...
test('Leaderboard aggregations', () => {
  ...
  mockStorage['cyberforge_best'] = '4600';
  mockStorage['cyberrift_best'] = '5000';
  mockStorage['cybercore_best'] = '5400';
  mockStorage['cyberflare_best'] = '5800';
  mockStorage['cybergrid_best'] = '6200';
  mockStorage['cyberprism_best'] = '6600';
  mockStorage['cybernova_best'] = '7000';
  mockStorage['cyberspectre_best'] = '7400';
  mockStorage['cybershadow_best'] = '7800';
  mockStorage['cyberhelix_best'] = '8200';
  mockStorage['cyberapex_best'] = '8600';
  mockStorage['cybertitan_best'] = '9000';
  mockStorage['cyberzenith_best'] = '9400';
  mockStorage['cybervanguard_best'] = '9800';
  mockStorage['cybersingularity_best'] = '10200';
  mockStorage['cybercascade_best'] = '10600';
  ...
  const cyberRiftScore = scores.find(s => s.id === 'cyberrift');
  const cyberCoreScore = scores.find(s => s.id === 'cybercore');
  const cyberPrismScore = scores.find(s => s.id === 'cyberprism');
  const cyberNovaScore = scores.find(s => s.id === 'cybernova');
  const cyberSpectreScore = scores.find(s => s.id === 'cyberspectre');
  const cyberShadowScore = scores.find(s => s.id === 'cybershadow');
  const cyberHelixScore = scores.find(s => s.id === 'cyberhelix');
  ...
  const cyberVanguardScore = scores.find(s => s.id === 'cybervanguard');
  const cyberSingularityScore = scores.find(s => s.id === 'cybersingularity');
  const cyberCascadeScore = scores.find(s => s.id === 'cybercascade');
  ...
  assert.strictEqual(cyberRiftScore.score, 5000);
  assert.strictEqual(cyberCoreScore.score, 5400);
  assert.strictEqual(cyberPrismScore.score, 6600);
  assert.strictEqual(cyberNovaScore.score, 7000);
  assert.strictEqual(cyberSpectreScore.score, 7400);
  assert.strictEqual(cyberShadowScore.score, 7800);
  assert.strictEqual(cyberHelixScore.score, 8200);
  assert.strictEqual(cyberApexScore.score, 8600);
  assert.strictEqual(cyberTitanScore.score, 9000);
  assert.strictEqual(cyberZenithScore.score, 9400);
  assert.strictEqual(cyberVanguardScore.score, 9800);
  assert.strictEqual(cyberSingularityScore.score, 10200);
  assert.strictEqual(cyberCascadeScore.score, 10600);
  ...
  assert.strictEqual(mockStorage['cyberrift_best'], undefined);
  assert.strictEqual(mockStorage['cybercore_best'], undefined);
  assert.strictEqual(mockStorage['cyberprism_best'], undefined);
  assert.strictEqual(mockStorage['cybernova_best'], undefined);
  assert.strictEqual(mockStorage['cyberspectre_best'], undefined);
  assert.strictEqual(mockStorage['cybershadow_best'], undefined);
  assert.strictEqual(mockStorage['cyberhelix_best'], undefined);
  ...
  assert.strictEqual(mockStorage['cybervanguard_best'], undefined);
  assert.strictEqual(mockStorage['cybersingularity_best'], undefined);
  assert.strictEqual(mockStorage['cybercascade_best'], undefined);
});
