...
test('CyberQuasarEngine - Accretion Shield activation', () => {
  const canvas = createMockCanvas();
  const engine = new CyberQuasarEngine(canvas);
  engine.start();
...
test('CyberQuasarEngine - Shooting relativistic plasma beams & update', () => {
  const canvas = createMockCanvas();
  const engine = new CyberQuasarEngine(canvas);
  engine.start();
  ...
  engine.shootBeam();
  assert.ok(engine.bullets.length >= 2);
...
test('CyberQuasarEngine - Game Over and high score update', () => {
  ...
  assert.strictEqual(engine.highScore, 6500);
});
