// Node.js Unit Tests for Cyber Kinetic Engine logic
...
// Extract CyberKineticEngine from cyberkinetic.html
const htmlPath = path.join(__dirname, '..', 'cyberkinetic.html');
const fileContent = fs.readFileSync(htmlPath, 'utf8');

const startIndex = fileContent.indexOf('class CyberKineticEngine {');
const endIndex = fileContent.indexOf('if (typeof module !== \'undefined\' && typeof module.exports !== \'undefined\')');
if (startIndex === -1 || endIndex === -1) {
  throw new Error('Could not find CyberKineticEngine boundaries in cyberkinetic.html');
}

const engineCode = fileContent.substring(startIndex, endIndex) + '\nmodule.exports = { CyberKineticEngine };';
...
const CyberKineticEngine = mockModule.exports.CyberKineticEngine;
...
test('CyberKineticEngine - Initial state', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
...
test('CyberKineticEngine - Start game & enemy spawning', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
...
test('CyberKineticEngine - EMP Nova shockwave trigger', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
  engine.start();
...
test('CyberKineticEngine - Shooting bullets and position update', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
  engine.start();
...
test('CyberKineticEngine - Game pause & resume toggle', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
  engine.start();
...
test('CyberKineticEngine - Game Over and high score update', () => {
  const canvas = createMockCanvas();
  const engine = new CyberKineticEngine(canvas);
  engine.start();
