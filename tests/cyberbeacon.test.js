// Node.js Unit Tests for Cyber Beacon Engine logic
...
// Extract CyberBeaconEngine from cyberbeacon.html
const htmlPath = path.join(__dirname, '..', 'cyberbeacon.html');
const fileContent = fs.readFileSync(htmlPath, 'utf8');

const startIndex = fileContent.indexOf('class CyberBeaconEngine {');
const endIndex = fileContent.indexOf('if (typeof module !== \'undefined\' && typeof module.exports !== \'undefined\')');
if (startIndex === -1 || endIndex === -1) {
  throw new Error('Could not find CyberBeaconEngine boundaries in cyberbeacon.html');
}

const engineCode = fileContent.substring(startIndex, endIndex) + '\nmodule.exports = { CyberBeaconEngine };';
...
const CyberBeaconEngine = mockModule.exports.CyberBeaconEngine;
...
test('CyberBeaconEngine - Initial state', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  ...
  assert.strictEqual(engine.beaconCore.hp, 100);
});

test('CyberBeaconEngine - Start game & enemy spawning', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
...
test('CyberBeaconEngine - EMP Pulse trigger', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  engine.start();
...
test('CyberBeaconEngine - Surge Shield activation', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  engine.start();
...
test('CyberBeaconEngine - Shooting beacon blasts and position update', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  engine.start();
  ...
  engine.shootBeaconBlast();
  assert.strictEqual(engine.bullets.length, 1);
  assert.strictEqual(engine.player.energy, initialEnergy - 4);
...
test('CyberBeaconEngine - Game pause & resume toggle', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  engine.start();
...
test('CyberBeaconEngine - Game Over and high score update', () => {
  const canvas = createMockCanvas();
  const engine = new CyberBeaconEngine(canvas);
  engine.start();
