...
test('CyberFluxEngine - Initial state', () => {
  ...
  assert.strictEqual(engine.player.polarity, 'positive');
});
...
test('CyberFluxEngine - Polarity shift toggle', () => {
  const canvas = createMockCanvas();
  const engine = new CyberFluxEngine(canvas);
  engine.start();
  ...
  assert.strictEqual(engine.player.polarity, 'negative');
  engine.togglePolarity();
  assert.strictEqual(engine.player.polarity, 'positive');
  assert.ok(engine.fluxShockwaves.length > 0);
});

test('CyberFluxEngine - EMP Flux Nova shockwave trigger', () => {
  const canvas = createMockCanvas();
  const engine = new CyberFluxEngine(canvas);
  engine.start();
