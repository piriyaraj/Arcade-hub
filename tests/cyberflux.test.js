...
test('CyberFluxEngine - Initial state', () => {
  ...
  assert.strictEqual(engine.player.polarity, 'POS');
});
...
test('CyberFluxEngine - Polarity swap toggle', () => {
  const canvas = createMockCanvas();
  const engine = new CyberFluxEngine(canvas);
  engine.start();
  ...
  assert.strictEqual(engine.player.polarity, 'NEG');
  engine.togglePolarity();
  assert.strictEqual(engine.player.polarity, 'POS');
});

test('CyberFluxEngine - EMP Shockwave trigger', () => {
  const canvas = createMockCanvas();
  const engine = new CyberFluxEngine(canvas);
  engine.start();
...
test('CyberFluxEngine - Game Over and high score update', () => {
  ...
  assert.strictEqual(engine.highScore, 2400);
});
